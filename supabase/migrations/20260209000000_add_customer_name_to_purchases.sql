-- Add customer_name column to customer_purchases table
ALTER TABLE public.customer_purchases 
ADD COLUMN IF NOT EXISTS customer_name TEXT;

-- Add index for faster lookups
CREATE INDEX IF NOT EXISTS idx_customer_purchases_customer_name 
ON public.customer_purchases(customer_name);

-- Drop and recreate sales_analytics view to include customer_name
-- We must drop because PostgreSQL doesn't allow changing column order with CREATE OR REPLACE
DROP VIEW IF EXISTS public.sales_analytics;
...
COMMENT ON VIEW public.sales_analytics IS 'Unified sales reporting view with reversal, quantity, customer sale rollups, payment tracking, and customer name.';

-- Grant access to the view
GRANT SELECT ON public.sales_analytics TO authenticated;

-- Update existing records: Set customer_name from customer_phone for walk-in customers for now
-- This is a one-time migration. Future records will have proper names.
UPDATE public.customer_purchases
SET customer_name = CASE 
  WHEN customer_phone = 'walk-in' THEN 'Walk-in Customer'
  WHEN customer_phone IS NULL THEN 'Walk-in Customer'
  ELSE customer_phone  -- Keep the phone number as name for now; it will be updated with actual names in future sales
END
WHERE customer_name IS NULL;

COMMENT ON COLUMN public.customer_purchases.customer_name IS 'Customer name for the purchase. Defaults to Walk-in Customer if not provided.';
