-- Final fix for all ambiguous column references
...
DECLARE
  ...
  inconsistency_score_var INTEGER;
...
BEGIN
  ...
  -- Detect inconsistencies and use different variable name
  inconsistency_score_var := detect_business_inconsistencies(user_uuid);
  ...
  -- Calculate new trust score with explicit parameter names and renamed variables
  new_score := calculate_trust_score_with_degradation(
    ...
    active_days, 
    debt_days_var, 
    inconsistency_score_var, 
    days_inactive
  );
  
  -- Insert or update trust score with renamed variables
  ...
  VALUES (
    ...
    debt_days_var, inconsistency_score_var, now(),
    (debt_days_var + inconsistency_score_var + (CASE WHEN days_inactive > 7 THEN days_inactive / 7 ELSE 0 END))::NUMERIC
  )
  ON CONFLICT (user_id) 
  DO UPDATE SET
    ...
    inconsistency_score = inconsistency_score_var,
    last_activity_date = now(),
    degradation_factor = (debt_days_var + inconsistency_score_var + (CASE WHEN days_inactive > 7 THEN days_inactive / 7 ELSE 0 END))::NUMERIC,
    last_updated = now();
END;
$function$;